...
def make_KS_loss(mask_size=3): # choose any mask size for calculating densities
    ...
    def D(X):
        ...
        x = tf.concat(X, 0)
        cdf1 = tf.cast(tf.searchsorted(x1, x, side='right'), dtype=tf.float32) / float(x1.shape[0])
        cdf2 = tf.cast(tf.searchsorted(x2, x, side='right'), dtype=tf.float32) / float(x2.shape[0])
    ...
    def my_KS_loss(y_true, y_pred):
        ...
        true_patches = tf.image.extract_patches(images=y_true,
                                   sizes=mask,
                                   strides=stride,
                                   rates=[1, 1, 1, 1],
                                   padding='SAME')
        pred_patches = tf.image.extract_patches(images=y_pred,
                                   sizes=mask,
                                   strides=stride,
                                   rates=[1, 1, 1, 1],
                                   padding='SAME')
