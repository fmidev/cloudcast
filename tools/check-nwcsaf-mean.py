...
def format_key(date):
    return "{}/{}/{}/{}_nwcsaf_effective-cloudiness".format(date.strftime("%Y"),
                date.strftime("%m"),
                date.strftime("%d"),
...
#            fp.write("{} with mean {:.3f} previous {} mean of means {:.3f} diff {:.3f}\n".format(nowtime, means[-1], len(means), window_mean, diff))
        msg = '{} with mean {:.3f} previous {} mean of means {:.3f} diff {:.3f}\n'.format(nowtime, means[-1], len(means), window_mean, diff)
        write_object('routines-data','cloudcast-source/nwcsaf/{}.flag'.format(format_key(nowtime)), msg)
        valid_dates = dates[0:-1]
        replace_dates.append(dates[-1])

    print("Valid dates for input are: {}".format(' '.join('{}'.format(k.strftime("%Y%m%dT%H%M%S")) for k in valid_dates)))

    if len(replace_dates) > 0:
        print("Invalid dates for input are: {}".format(' '.join('{}'.format(k.strftime("%Y%m%dT%H%M%S")) for k in replace_dates)))

    if plot:
        import matplotlib.pyplot as plt

        img = datas[0]
        fig, axes = plt.subplots(nrows=1, ncols=len(means), figsize=(10, 6),
