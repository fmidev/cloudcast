...
def get_filename(time, producer = 'nwcsaf', analysis_time=None, param='effective-cloudiness'):
    ...
    if producer == 'meps':
        ...
        lt_h = int(lt.total_seconds() // 3600)
        lt_m = int(lt.total_seconds() // 60 % 60)
        return '{}/meps/{}/{}{:02d}00+{:03d}h{:02d}m-{}.grib2'.format(INPUT_DIR, analysis_time.strftime('%Y/%m/%d'), analysis_time.strftime('%Y%m%d'), ahour, lt_h, lt_m, param)
    if producer == 'DEM':
...
def read_time(time, producer='nwcsaf', analysis_time=None, param='effective-cloudiness', **kwargs):
