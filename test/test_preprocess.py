...
from base.gributils import read_grib
...
def test_create_datetime():
    ...
    for d in np.arange(365 * 24):
        ...
        dt = parser.parse("2022-01-01 00:00:00 UTC") + datetime.timedelta(
            hours=(int(d))
        )
...
def test_create_sun_elevation_angle():
    ...
    ts = parser.parse("2022-06-30 00:00:00+00")
    grid = create_sun_elevation_angle(ts, (128, 128))
    plt.subplot(2, 2, 1)
    plt.xlabel(ts)
    plt.imshow(np.squeeze(grid))

    ts = parser.parse("2022-06-30 06:00:00+00")
    grid = create_sun_elevation_angle(ts, (128, 128))
    plt.subplot(2, 2, 2)
    plt.xlabel(ts)
    plt.imshow(np.squeeze(grid))

    ts = parser.parse("2022-06-30 12:00:00+00")
    grid = create_sun_elevation_angle(ts, (128, 128))
    plt.subplot(2, 2, 3)
    plt.xlabel(ts)
    plt.imshow(np.squeeze(grid))

    ts = parser.parse("2022-06-30 18:00:00+00")
    grid = create_sun_elevation_angle(ts, (128, 128))
    plt.subplot(2, 2, 4)
    plt.xlabel(ts)
    plt.imshow(np.squeeze(grid))
...
def test_pca():
    time = datetime.datetime.strptime("2020-05-10 12:00:00", "%Y-%m-%d %H:%M:%S")
    data = read_grib(get_filename(time), img_size=(64, 64))
